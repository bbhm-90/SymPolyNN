...
def read_and_plot(data_sym_res_dir, loss_th=None):
    for sr_dir in os.listdir(data_sym_res_dir):
        ...
        for i in range(totNumEq):
            ...
            ax.plot(x, y_pred, label=f"symbolic eq. (complx. {label})")
            ...
            if loss_th is not None:
                break
            eq_rec[f'eq_complex_{label}'] = y_pred.flatten()
...
if __name__ == "__main__":
    ...
    base_dir = "example/MatsuokaNakai/step_1_nn_training/results_21/cylindrical/PolynomialHO/PosEnc_0/shape_func"
